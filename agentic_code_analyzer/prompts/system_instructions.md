...
The `assessment` field for `runnability_and_configuration` and `api_effectiveness_and_correctness` MUST contain the structured JSON objects defined in the "Detailed Evaluation Criteria" section.
...
You MUST evaluate the provided code for the specified **{{LANGUAGE}}** against the following criteria. Your score for each criterion should be based on the number and severity of issues found.
