...
def test_safe_json_load(input_data, expected_output, test_id):
    ...
    assert agent._safe_json_load(input_data) == expected_output, f"Test failed for: {test_id}"
        ...
        (
            {
                "criteria_breakdown": [
                    ...
                    },
                    {
                        ...
                        "recommendations_for_llm_fix": ["rec_A", "rec_C"], # rec_A is a duplicate
                    },
                ]
            },
            {
...
def test_enforce_single_penalty_hierarchy(input_data, expected_output, test_id):
    ...
    if "criteria_breakdown" in result and isinstance(result["criteria_breakdown"], list):
        # The agent internally sorts, but we sort here again to be safe
        result["criteria_breakdown"] = sorted(result["criteria_breakdown"], key=lambda x: x.get("criterion_name", "") if isinstance(x, dict) else "")
    if "criteria_breakdown" in expected_output and isinstance(expected_output["criteria_breakdown"], list):
        expected_output["criteria_breakdown"] = sorted(expected_output["criteria_breakdown"], key=lambda x: x.get("criterion_name", "") if isinstance(x, dict) else "")
