...
async def run_agent_test(initial_state: dict, mocked_llm_response: str, mocker):
    ...
    async for event in agent._run_async_impl(context):
        if event.is_final_response() and event.content and event.content.parts:
            context.session.state["evaluation_review_agent_output"] = event.content.parts[0].text
...
class TestJsonFormattingAgent:
    ...
    async def test_agent_stores_llm_response_in_session(self, llm_response, test_id, mocker):
        """
        ...
        assert result_str is not None, "The output key should be populated in the session state"
