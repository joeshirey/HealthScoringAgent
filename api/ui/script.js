...
analyzeBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const validationHistory = data.validation_history;
    ...
    overallList.innerHTML = `
            ...
            <li><b>Region Tags:</b> ${analysis.region_tags ? analysis.region_tags.join(", ") : "N/A"}</li>
        `;
    ...
    validationTitle.textContent = "Evaluation Validation History";
    validationSection.appendChild(validationTitle);

    validationHistory.forEach((attempt, index) => {
      const attemptDiv = document.createElement("div");
      attemptDiv.classList.add("validation-attempt");

      const attemptTitle = document.createElement("h3");
      attemptTitle.textContent = `Attempt ${index + 1}`;
      attemptDiv.appendChild(attemptTitle);

      const validationList = document.createElement("ul");
      validationList.innerHTML = `
                <li><b>Validation Score:</b> ${attempt.validation_score}/10</li>
                <li><b>Reasoning:</b> ${attempt.reasoning}</li>
            `;
      attemptDiv.appendChild(validationList);
      validationSection.appendChild(attemptDiv);
    });
    ...
    evaluationTitle.textContent = "Final Detailed Evaluation";
    evaluationSection.appendChild(evaluationTitle);
    const evaluationContent = document.createElement("pre");
    evaluationContent.textContent = JSON.stringify(
      analysis.evaluation,
      null,
      2,
    );
